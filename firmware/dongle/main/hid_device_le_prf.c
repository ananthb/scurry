...
bool hidd_clcb_dealloc (uint16_t conn_id)
{
    ...
    /* scurry: upstream unconditionally cleared the FIRST slot and returned,
       ignoring conn_id entirely. Harmless while HID_MAX_APPS was 1; with a
       real array it means disconnecting one host wipes another host's slot.
       Match on conn_id and only clear the slot that actually went away. */
    for (i_clcb = 0, p_clcb= hidd_le_env.hidd_clcb; i_clcb < HID_MAX_APPS; i_clcb++, p_clcb++) {
